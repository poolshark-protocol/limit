    ...
    function insert(
        ...
        if (!params.zeroForOne || cache.priceLower > cache.pool.price) {
            ...
            if(!TickMap.set(tickMap, params.lower, cache.constants.tickSpacing)){
                // inherit epoch 
                console.log('lower tick being set first time');
                int24 tickAhead;
                if (params.zeroForOne) {
                    tickAhead  = TickMap.next(tickMap, params.lower, cache.constants.tickSpacing, true);
                } else {
                    tickAhead  = TickMap.previous(tickMap, params.lower, cache.constants.tickSpacing, true);
                }
        ...
        if (params.zeroForOne || cache.priceUpper < cache.pool.price) {
            if(!TickMap.set(tickMap, params.upper, cache.constants.tickSpacing)) {
                ...
                int24 tickAhead;
                if (params.zeroForOne) {
                    tickAhead  = TickMap.next(tickMap, params.upper, cache.constants.tickSpacing, true);
                } else {
                    tickAhead  = TickMap.previous(tickMap, params.upper, cache.constants.tickSpacing, true);
                }
                uint32 epochAhead = EpochMap.get(tickAhead, tickMap, cache.constants);
                console.log('setting epoch', epochAhead, uint24(tickAhead), TickMap.get(tickMap, 16005, cache.constants.tickSpacing));
