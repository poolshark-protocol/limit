...
describe('RangePool Exact In Tests', function () {
  ...
  it('token1 - Should mint, swap, and burn 29', async function () {
  ...
  it('token0 - Should mint, swap, and burn 29', async function () {
    const pool: RangePoolState = (await hre.props.limitPool.globalState()).pool
  ...
  it('token0 - Should mint and burn fungible position 29', async function () {
    const pool: RangePoolState = (await hre.props.limitPool.globalState()).pool
 ...
 //   if (debugMode) await getSample()
  ...
  it('token0 - Should add in-range fungible liquidity 29', async function () {
    const pool: RangePoolState = (await hre.props.limitPool.globalState()).pool
    await validateMint({
      signer: hre.props.alice,
      recipient: hre.props.alice.address,
 ...
 //   if (debugMode) await getSample()
  ...
  it('token1 - Should mint, swap, and burn position while in range 29', async function () {
    if (debugMode) await getTickAtPrice()
 ...
 //   if (debugMode) await getSample()
  ...
  it('token0 - Should autocompound fungible position 29', async function () {
    const pool: RangePoolState = (await hre.props.limitPool.globalState()).pool
 ...
 //   if (debugMode) await getSample()
  ...
  it('token0 - Should autocompound fungible position and add liquidity 29', async function () {
 ...
 //   if (debugMode) await getSample()
  ...
  it('pool - Should not skip crossing tickAtPrice :: KEBABSEC', async function () {
    const pool: RangePoolState = (await hre.props.limitPool.globalState()).pool
    ...
    await validateBurn({
      ...
      //TODO: user loses deposit after...it shouldn't transfer the extra balance in the first place
      //this is probably because of where the price is at
      //Positions.resize should handle transferring in the correct amount
      revertMessage: '',
    })
    if (balanceCheck) {
      console.log('balance after token0:', (await hre.props.token0.balanceOf(hre.props.limitPool.address)).toString())
...
describe('RangePool Exact Out Tests', function () {
 ...
 //   if (debugMode) await getSample()
  ...
  it('token0 - Should mint, swap, and burn 27', async function () {
    ...
    const aliceLiquidity = BigNumber.from('55483175795606442088585')
 ...
 //   if (debugMode) await getSample()
  ...
  it('token0 - Should add in-range fungible liquidity 27', async function () {
    ...
    await validateSwap({
      ...
      sqrtPriceLimitX96: maxPrice,
      balanceInDecrease: BigNumber.from('99999999999999999999'),
      balanceOutIncrease: BigNumber.from('32121736932093337716'),
      revertMessage: '',
      exactIn: false
    })
 ...
 //   if (debugMode) await getSample()
    ...
    await validateBurn({
      ...
      liquidityAmount: BigNumber.from('419027207938949970576'),
      balance0Increase: BigNumber.from('67878263067906662282'),
      balance1Increase: BigNumber.from('100000000000000000000').sub(2),
      revertMessage: '',
    })
  ...
  it('token1 - Should mint, swap, and burn 27', async function () {
    ...
    await validateSwap({
      ...
      sqrtPriceLimitX96: maxPrice,
      balanceInDecrease: BigNumber.from('9999999999999999998'),
      balanceOutIncrease: BigNumber.from('1345645380966504669'),
      revertMessage: '',
      exactIn: false
    })
 //   if (debugMode) await getSample()
    ...
    await validateBurn({
      ...
      liquidityAmount: liquidityAmount2,
      balance0Increase: BigNumber.from('98654354619033495329'),
      balance1Increase: tokenAmount.div(10).sub(3),
      revertMessage: '',
    })
  ...
  it('token1 - Should mint, swap, and burn position while in range 27', async function () {
    if (debugMode) await getTickAtPrice()
    const aliceLiquidity = BigNumber.from('1577889144107833733009')
    const aliceLiquidity2 = BigNumber.from('1590926220637829792698')
    const aliceTokenAmount = BigNumber.from('3168808846234106973656')
    await validateMint({
      signer: hre.props.alice,
      recipient: hre.props.alice.address,
      lower: '25000',
    ...
    await validateSwap({
      ...
      sqrtPriceLimitX96: maxPrice,
      balanceInDecrease: BigNumber.from('9999999999999999993'),
      balanceOutIncrease: BigNumber.from('819054826219841040'),
      revertMessage: '',
      exactIn: false
    })
 ...
 //   if (debugMode) await getSample()
    ...
    await validateBurn({
      ...
      liquidityAmount: aliceLiquidity.add(aliceLiquidity2),
      balance0Increase: BigNumber.from('199180945173780158958'),
      balance1Increase: BigNumber.from('20082623526365456109'),
      revertMessage: '',
    })
  ...
  it('token0 - Should autocompound fungible position 27', async function () {
    const pool: RangePoolState = (await hre.props.limitPool.globalState()).pool
    const aliceLiquidity = BigNumber.from('3852877204305891777654')
    const aliceToken2 = BigNumber.from('7703654602399898969712')
    const aliceLiquidity2 = BigNumber.from('7705754408611783555386')
    await validateMint({
      signer: hre.props.alice,
      recipient: hre.props.alice.address,
      lower: '500',
 ...
 //   if (debugMode) await getSample()
    await validateBurn({
      ...
      liquidityAmount: aliceLiquidity.add(aliceLiquidity2),
      balance0Increase: BigNumber.from('150000000000000000000').sub(1),
      balance1Increase: BigNumber.from('136483615696225509031'),
      revertMessage: '',
    })
  ...
  it('token0 - Should autocompound fungible position and add liquidity 27', async function () {
    const aliceLiquidity = BigNumber.from('7705754408611783555386')
    const aliceLiquidity2 = BigNumber.from('3852877204305891777654')
    const aliceToken2 = BigNumber.from('3851318661512648798121')
 ...
 //   if (debugMode) await getSample()
    ...
    await validateBurn({
      ...
      upper: '1000',
      tokenAmount: aliceLiquidity.add(aliceToken2),
      liquidityAmount: BigNumber.from('11559154372605880114689'), //TODO: investigate
      balance0Increase: BigNumber.from('62500000000000000'),
      balance1Increase: BigNumber.from('300013568033977842762'),
      revertMessage: '',
    })
 //   if (debugMode) await getSample()
    if (balanceCheck) {
  ...
  it('pool - Should mint position inside the other 27', async function () {
    ...
    await validateSwap({
      ...
      amount: tokenAmount,
      sqrtPriceLimitX96: BigNumber.from('82255474610179467046984074964'),
      balanceInDecrease: BigNumber.from('8404133769503785679'), // token1 increase in pool
      balanceOutIncrease: BigNumber.from('7801206245756322178'), // token0 decrease in pool
      revertMessage: '',
      exactIn: false
    })
  ...
  it('pool - Should not underflow when crossing when exiting and entering position range 27:: KEBABSEC', async function () {
    ...
    const aliceTokenAmount3 = BigNumber.from('289243963978185495685')
    const aliceLiquidity3 = BigNumber.from('289244426214719608262')
    const aliceTokenAmount4 = BigNumber.from('867731891934556487058')
    const aliceLiquidity4 = BigNumber.from('867733278644158824787')
    ...
    await validateSwap({
      ...
      sqrtPriceLimitX96: BigNumber.from('3077067665772502614228114341887'),
      balanceInDecrease: BigNumber.from('1000119999999999999658'), // token1 increase in pool
      balanceOutIncrease: BigNumber.from('666077964566722076'), // token0 decrease in pool
      revertMessage: '',
      exactIn: false
    })
    ...
    await validateSwap({
      ...
      sqrtPriceLimitX96: BigNumber.from('3077067665772502614228114341887'),
      balanceInDecrease: BigNumber.from('1000119999999999999405'), // token1 increase in pool
      balanceOutIncrease: BigNumber.from('665958920765498692'), // token0 decrease in pool
      revertMessage: '',
      exactIn: false
    })
    ...
    await validateSwap({
      ...
      sqrtPriceLimitX96: BigNumber.from('3077754393416758970167012098048'),
      balanceInDecrease: BigNumber.from('1496453378999999999987'), // token1 increase in pool
      balanceOutIncrease: BigNumber.from('996234651079624794'), // token0 decrease in pool
      revertMessage: '',
      exactIn: false
    })
    ...
    await validateBurn({
      ...
      liquidityAmount: BigNumber.from('289244763213965801676'),
      balance0Increase: BigNumber.from('1000001165101954093'),
      balance1Increase: BigNumber.from('1998742875900707205'),
      revertMessage: '',
    })
    //console.log(0xf56c6732aec4578f3068358f213ac61548744890c9b59a9b4d94deea7095e007)
    await validateMint({
    ...
    await mintSigners20(hre.props.token0, BigNumber.from('33023622513667392995848'), [hre.props.alice, hre.props.bob])
    await mintSigners20(hre.props.token1, BigNumber.from('33023622513667392995848'), [hre.props.alice, hre.props.bob])
    const aliceTokenAmount6 = BigNumber.from('2879560692345943491092')
    const aliceLiquidity6 = BigNumber.from('2879560960970776983363')
    await validateMint({
      signer: hre.props.alice,
      recipient: hre.props.alice.address,
      lower: '66120',
    ...
    await validateSwap({
      ...
      sqrtPriceLimitX96: BigNumber.from('3059588122704193012744629256192'),
      balanceInDecrease: BigNumber.from('815798405335420361'), // token1 increase in pool
      balanceOutIncrease: BigNumber.from('1221404429444282148277'), // token0 decrease in pool
      revertMessage: '',
      exactIn: false
    })
    // await getTickAtPrice()
    ...
    await validateSwap({
      ...
      sqrtPriceLimitX96: BigNumber.from('3075057850633752459897890406400'),
      balanceInDecrease: BigNumber.from('4999999999999998721'), // token1 increase in pool
      balanceOutIncrease: BigNumber.from('3351015920029701'), // token0 decrease in pool
      revertMessage: '',
      exactIn: false
    })
    ...
    //console.log('0x4950b3696a62cdc4e9584c81911a4c3b6f6cc5c4013ea8454286a27d150d6f69')
    const aliceTokenAmount7 = BigNumber.from('621480049120650311480')
    const aliceLiquidity7 = BigNumber.from('621480049120650311480')
    await validateMint({
      signer: hre.props.alice,
      recipient: hre.props.alice.address,
      lower: '66960',
      ...
      balance0Decrease: BigNumber.from('5000000000000000000'),
      balance1Decrease: BigNumber.from('6324961650864055083553'),
      tokenAmount: aliceTokenAmount7,
      liquidityIncrease: aliceLiquidity7,
      revertMessage: '',
      collectRevertMessage: ''
    ...
    await validateBurn({
      ...
      balance0Increase: BigNumber.from('1006006157700985848708'),
      balance1Increase: BigNumber.from('1749018723452173171'),
      revertMessage: '',
    })
    await validateBurn({
      signer: hre.props.alice,
      ...
      upper: '80160',
      burnPercent: ethers.utils.parseUnits('1', 38),
      liquidityAmount: BigNumber.from('5890070187730690086991'),
      balance0Increase: BigNumber.from('45478016986915742292'),
      balance1Increase: BigNumber.from('66834894850190234086497'),
      revertMessage: '',
    })
    await validateBurn({
      signer: hre.props.alice,
      ...
      liquidityAmount: aliceLiquidity7,
      balance0Increase: BigNumber.from('4999999999999999999'),
      balance1Increase: BigNumber.from('6324961650864055083543'),
      //TODO: user loses deposit after...it shouldn't transfer the extra balance in the first place
      //this is probably because of where the price is at
      //Positions.resize should handle transferring in the correct amount
      revertMessage: '',
